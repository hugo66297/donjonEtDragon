<div class="relative flex flex-col flex-1">
    <div class="flex justify-between items-center px-2 sm:px-8">
        <div>
        ...
        <div>
            ...
            <div class="relative">
                <input
                    type="range"
                    wire:model="range.values.0"
                    min="{{ $range['min'][0] }}"
                    max="{{ $range['max'][0] }}"
                    step="1"
                    class="w-64 h-1 bg-gray-300 rounded-full appearance-none"
                >
                <input
                    type="range"
                    wire:model="range.values.1"
                    min="{{ $range['min'][1] }}"
                    max="{{ $range['max'][1] }}"
                    step="1"
                    class="w-64 h-1 bg-gray-300 rounded-full appearance-none"
                ...
                <span>Min: {{ $range['values'][0] ?? '' }}</span>
                <span>Max: {{ $range['values'][1] ?? '' }}</span>
            </div>
        </div>
        <div>
            <div class="flex items-center">
                <input
                    ...
                    wire:model="isRituel"
                >
                <label for="bordered-checkbox-2" class="w-full ml-2 text-sm font-medium text-gray-900">Sort de rituel</label>
            </div>
        </div>
    </div>
    ...
    <ul
        role="list"
        class="px-2 sm:px-8 sm:grid sm:gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5"
    >
