@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center my-2">
        <div class="flex justify-center space-x-8 flex-1 sm:hidden">
            @if ($paginator->onFirstPage())
                <span class="relative inline-flex items-center px-4 py-2 text-sm font-bold text-gray-500 bg-white border border-gray-300 cursor-default leading-5 rounded-md">
{{--                    {!! __('pagination.previous') !!}--}}
                    &laquo; Précédent
                </span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-bold text-gray-800 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
{{--                    {!! __('pagination.previous') !!}--}}
                    &laquo; Précédent
                </a>
            @endif
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-bold text-gray-800 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
{{--                    {!! __('pagination.next') !!}--}}
                    Suivant &raquo;
                </a>
            @else
...
{{--                    {!! __('pagination.next') !!}--}}
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:flex-col space-y-2">
